...
if (!isset($_POST['user_id'])) {
    $_SESSION['message'] = "User ID is not set in POST request";
    exit();
}
...
if (mysqli_stmt_execute($stmt)) {
    ...
    if (mysqli_stmt_execute($stmt)) {
        // Commit transaction
        mysqli_commit($con);
        $_SESSION['message'] = "User and related notes deleted successfully";
        header("Location: ../admin_dash.php");
        exit();
    } else {
        // Rollback transaction
        mysqli_rollback($con);
        $_SESSION['message'] = "Failed to delete related notes";
    }
} else {
    // Rollback transaction
    mysqli_rollback($con);
